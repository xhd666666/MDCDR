...
for seed in seed_list:
    ...
    train_set, val_set, test_set = load_leave_mixed_split(df_gdsc=gdsc_response, df_tcga=tcga_response,
                                                 random_state=seed, group_col=mode)

    print(f"Mode {mode}-{seed} data distribution:")
    print(
        f"  Training set: {len(train_set)} samples | Validation set: {len(val_set)} samples | Test set: {len(test_set)} samples")

    train_dataset = MyDataset(drug_dict, cell_dict, train_set)
    val_dataset = MyDataset(drug_dict, cell_dict, val_set)
    ...
    train(args, model, train_loader, val_loader, test_loader, seed, mode = mode)